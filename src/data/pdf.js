...
const lorem =
  "Wellbeing is a digital health and wellness platform that provides physical fitness services such as calorie tracking";
...
async function pdfDoc(email) {
  ...
  doc.text("WELLBEING", { align: "center" }).font("Times-Roman").fontSize(25);

  doc.moveDown(5).font("Times-Roman").fontSize(10).text(lorem, 100, 100);

  doc
    .addPage()
    .font("Times-Bold")
    .fontSize(16)
    .text(`${JSON.stringify(water.name)} - Today's Water History`, {
      align: "center",
    });
  ...
  if (waterArc) {
    doc
      ...
      .font("Times-Bold")
      .fontSize(16)
      .text(`${JSON.stringify(water.name)} - Past Water History`, {
        align: "center",
      });

    doc.moveDown(3).fontSize(12).font("Times-Roman").text(waterArc, 100, 100);
  }
  ...
  if (res) {
    doc
      ...
      .font("Times-Bold")
      .fontSize(16)
      .text(`${JSON.stringify(water.name)} - Food History`, {
        align: "center",
      });

    doc.fontSize(12).font("Times-Roman").text(res, 100, 100);
  }
